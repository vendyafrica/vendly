...
export async function GET() {
    ...
    try {
        ...
        const [newTenants7d] = await db.select({ count: count() }).from(tenants).where(gte(tenants.createdAt, sevenDaysAgo));
        const [newTenants30d] = await db.select({ count: count() }).from(tenants).where(gte(tenants.createdAt, thirtyDaysAgo));

        // 2. Store Metrics
        const [totalStores] = await db.select({ count: count() }).from(stores);
        const [activeStores] = await db.select({ count: count() }).from(stores).where(eq(stores.status, true));
        const [inactiveStores] = await db.select({ count: count() }).from(stores).where(eq(stores.status, false));

        // 3. Marketplace Metrics
        // GMV (Paid Orders)
        const [gmvResult] = await db.select({ total: sum(orders.totalAmount) })
            .from(orders)
        ...
        const topStoresByRevenue = await db.select({
        ...
        const topStoresByVisits = await db.select({
