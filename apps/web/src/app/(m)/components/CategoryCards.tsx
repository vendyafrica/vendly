...
export default function InteractiveImageBentoGalleryDemo({ categories = [] }: CategoryCardsProps) {
  let imageItems = FALLBACK_ITEMS;

  if (categories.length > 0) {
    imageItems = categories.slice(0, 6).map((cat, index) => ({
      id: cat.id,
      title: cat.name,
      desc: "",
